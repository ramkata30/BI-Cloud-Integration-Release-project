...
# Architecture
<img width="940" height="627" alt="image" src="https://github.com/user-attachments/assets/0e30fd0c-18f6-4482-a429-26b0ccd3e289" />

# Key Notes
# Authentication
- Preferred: OIDC (aws-actions/configure-aws-credentials) assuming an IAM role with Redshift access.
- Fallback: Store REDSHIFT_HOST, REDSHIFT_DB, REDSHIFT_USER, REDSHIFT_PASSWORD as GitHub Secrets.
# SQL Ordering
- If execution_order is provided → Executes in that order.
- If single file path → Runs just that file.
- If folder path → Executes alphabetically (unless overridden).
# Error Handling
